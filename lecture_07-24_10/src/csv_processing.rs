...
pub fn create_csv_table(input: &str) -> String {
    let mut csv_table = CsvTable::new();
    let mut buffer = vec![]; // TODO: with capacity based on len of input

    input.lines().for_each(|line| {
        if let Err(error) = writeln!(csv_table, "{}", line) {
            eprintln!("Error during creating csv table: {error}.");
        }
    });

    if let Err(error) = csv_table.output_to(&mut buffer) {
        eprintln!("Error during outputting csv table: {error}.");
    }

    String::from_utf8(buffer).unwrap_or("[CANNOT CREATE CSV TABLE]".into())
}
